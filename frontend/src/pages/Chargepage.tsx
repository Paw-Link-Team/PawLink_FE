...
export default function Chargepage() {
  ...
  const addDigits = (v: string) => {
    // v는 "1"~"9" or "0" or "00"
    const next = (digits + v).replace(/^0+(\d)/, "$1"); // 앞자리 0 정리
    // 너무 길어지는 거 방지(필요하면 조절)
      ...
      <div className="cp-phone">
        ...
        <main className="cp-main">
          ...
          <section className="cp-keypad" aria-label="keypad">
            ...
            <div className="cp-key-row">
              ...
              <button className="cp-key cp-key-back" onClick={backspace} type="button" aria-label="backspace">
                ←
              </button>
            </div>
          </section>
        ...
        {(step === "confirm" || step === "done") && (
          <div className="cp-dim" onClick={() => setStep("input")} role="presentation">
            <div className="cp-sheet" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
